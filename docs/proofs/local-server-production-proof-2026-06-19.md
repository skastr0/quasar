...
- Server URL: `http://100.64.0.1:6180`
...
docker compose --env-file platform/local-server/.env \
  ...
  "cd /app && timeout 240s bun packages/local-server/src/cli.ts maintain --vector true --optimize true"
```
...
```bash
curl -fsS http://100.64.0.1:6180/search/fusion \
  --get \

...
- Semantic/fusion search is disabled pending vector materialization ;
...
| Search | `search --query <text> --mode lexical\|semantic\|fusion` | `GET /search/<mode>` | `q`/`query`, `projectKey`, `role=user\|assistant`, `limit`; `semantic`/`fusion` return 503 `SemanticDisabled` pending work-item |
...
`passed` is true, and the report records the exact threshold used. The work-item

...
- Access is by direct Tailscale IP first, e.g. `http://100.64.0.1:6180`.
...
5. From another Tailnet client, verify direct Tailscale IP access:
   ...
   ```bash
   curl -fsS http://100.64.0.1:6180/health
   curl -fsS http://100.64.0.1:6180/status
   ```
...
```cron
*/15 * * * * cd /home/user/Projects/quasar && /opt/homebrew/bin/bun run local-server:sync-tick >> logs/local-server-sync.log 2>&1
```

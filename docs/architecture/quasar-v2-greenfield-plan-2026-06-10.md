...
Provenance: synthesized from a 14-agent forensic audit (tower glyph history the current work sequence,
...
- Distance to a working full-corpus ingest + search: ~6-10 focused glyphs, 3-5 days at the
  demonstrated pace.
- The 1.5x wire-amplification gate invented by work-item is mathematically unreachable in the
  ...
  rot. Continuing the the current work sequence compaction chain cannot pass and is gutting product
...
3. **Idempotency: A per-record hash-skip vs B key-based always-write.** RULING: B. The hash-skip-before-refresh path *is* the work-item stale-search-doc bug; always-write makes the class unrepresentable.
...
= id scheme (B)
  ...
  | project key `p` :: base32(60-bit hash of canonical path) = 12 chars, **immutable by construction**. Aliasing/grouping is a read-time table, never a stored-row rewrite — deletes the work-item class.
...
= ingest endpoint :: `POST /sync` — single write path. Fail-closed schema decode, 512 KiB pre-parse cap (reuse http.ts:52-125). Apply: ses upsert by token → msg/tc upsert by (s,q)/(s,c) with a **per-envelope memoized session-lookup Map** (fixes 200 identical reads) → trims → dels.
  **out-of-order tolerant** :: msg before ses creates a stub session row, filled when ses arrives — envelopes valid in any order (deletes v1's fail-closed-parent poison). **No unchanged-hash skip path exists** — idempotency is key-based, so work-item is unrepresentable.
...
! 6 gate criteria before product criteria :: fidelity sacrificed to pass a ratio whose denominator shrinks with the cuts
  ^ work-item clamped tool payloads to 256B/160-char previews, gutting the stated goal "search over tool-call inputs/outputs"; working-tree measurement: 3.17x persisted because useful text shrank as fast as wire
...
! 8 hash skip without identity closure :: idempotency keyed on partial content while writes denormalize more than the hash covers
  ^ work-item failure — child hashes parent-invariant + unchanged-skip + parent identity stamped into child rows/search docs → silent staleness; same family: alias repoint capped at 500 rows, no continuation
...
! 9 patching the dying plane :: crisis patches stacked on architecture already known structurally broken
  ^ the current work sequence: four glyphs in one morning on the blob plane, all abandoned 20h later; Era 2 = 57 commits, 95% fix:, 1,208-line module lived <23h
...
! 13 rubber stamp review era :: reviews structurally present, functionally absent, exactly while the doomed plane was built
  ^ reviewing→done in 0-22s (work-item/006/007); reviewers timed out without verdicts (work-item/012/013); post-reset real reviews immediately caught real bugs (data loss, staleness)
...
|- crash resume :: kill -9 mid-ingest, rerun converges to byte-identical server state with zero duplicate writes — the work-item proof exists and is green from the first vertical slice
...
= glyph sizing :: one falsifiable claim + its measurement per glyph
  work-item (whole product, "done" in 2h) and work-item (done 33s after creation) were bookkeeping; 9 glyphs authored in 3 minutes is batch planning with zero feedback between units

= gate placement :: the measurement gate goes FIRST, not seventh
  work-item was created after six layers landed; the 5.33x first measurement should have halted the arc — a gate that can only fail after everything is built is a post-mortem; work-item (the only end-to-end product proof) never left backlog while 24/28 glyphs went to ingest
...
= review rigor precedes velocity, not the reverse
  the rubber-stamp era built the doomed plane; the moment reviews became real they caught data loss  and staleness  — treat review capacity as the throughput ceiling and pace authoring to it

= reset quality was the one bright spot :: replicate it
  work-item worked because it was destructive (zero file-path reuse), vocabulary-banning at the concept level, and accompanied by a written post-mortem — partial salvage (Era 2 patches) failed; clean conceptual resets with explicit principle docs succeeded

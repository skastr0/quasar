...
work-item repo reorientation (this document) → work-item Convex core →
work-item ingest engine + claude proof (blocks all optimization) → work-item codex +
cross-provider project unity → work-item opencode + garbage-boundary proof →
work-item hermes/grok → work-item validation swarm (reconciliation, consumption audit,
guideline conformance, pinned relevance — all executable) → work-item Gemini embeddings →
work-item MCP/Tailscale serving → work-item ops.
...
the tree at work-item; mine git history if ever needed.

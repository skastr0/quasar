...
> 4 MiB/s boundary is a non-event at sequential pace). work-item/044 were abandoned
> with provenance; the replacement sequence is the current work sequence.
...
This is a Convex rejection, so work-item halted. The next implementation must not
...
No work-item or work-item implementation commit should land until these decisions

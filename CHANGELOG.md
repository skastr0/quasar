...
- The daemon polls Amp's servers on a throttle instead of every tick: one
  full thread list every 20 minutes, then exports only changed threads. State
  is one timestamp in `amp-poll-state.json` beside the ingest manifest.
  `ingest --provider amp` still lists immediately.

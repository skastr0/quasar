...
// ---------------------------------------------------------------------------
// Schema-driven fixture constructors (work-item FULL DATA FIDELITY).
//
...
// ---------------------------------------------------------------------------
// T2 : multi-agent session UN-MERGED into first-class sessions.
//
...
// ---------------------------------------------------------------------------
describe("T2 : subagents become first-class sessions with lineage", () => {
...
// ---------------------------------------------------------------------------
// T6 : wire records are decoded fail-closed.
//
...
// ---------------------------------------------------------------------------
describe("T6 : malformed wire record → named diagnostic + dropped, valid records survive", () => {
...
// ---------------------------------------------------------------------------
// T7 (work-item review FIX B): an agents/<id>/wire.jsonl present ON DISK but
...
// ---------------------------------------------------------------------------
describe("T7 (work-item FIX B): undeclared on-disk wire → named diagnostic + ingested orphan", () => {
...
// ---------------------------------------------------------------------------
// T8 (work-item review FIX C): a sub-agent spawned by ANOTHER sub-agent must link
// to that sub's own compound canonical id — NOT flattened onto main. The edge
// fromId honours agent.parentAgentId.
// ---------------------------------------------------------------------------
describe("T8 (work-item FIX C): nested parentAgentId links to the real parent sub, not main", () => {
...
// ===========================================================================
// T9 (work-item FULL DATA FIDELITY): declarative signal/drop dispatch — ONE case
...
// ===========================================================================
describe("T9 : per-record-type signal/drop classification", () => {
  type Verdict =
    | { signal: string }
    | { drop: string };
...
// ===========================================================================
// T10 : full-spectrum wire through the ADAPTER. A single agent wire
...
// ===========================================================================
describe("T10 : full-spectrum wire → only signals become events, drops vanish", () => {
...
// ===========================================================================
// T11 : malformed-record fail-closed per MAJOR type. A structurally
...
// ===========================================================================
describe("T11 : malformed records → named diagnostic + drop, no throw", () => {
...
// ===========================================================================
// T12 : the boundary schema is a CLOSED union — round-trip decode of a
// schema-built record succeeds, and an unmodeled outer type fails decode.
// ===========================================================================
describe("T12 : KimiWireRecordSchema is a closed fail-closed union", () => {

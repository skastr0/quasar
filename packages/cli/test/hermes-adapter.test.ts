...
// ---------------------------------------------------------------------------
// work-item FULL DATA FIDELITY — fixtures are built FROM the Effect schemas via
...
// ---------------------------------------------------------------------------
// work-item: subagent parent lineage
//
...
// ---------------------------------------------------------------------------
describe("work-item: subagent_of lineage edge → canonical parentSessionId", () => {
...
// ---------------------------------------------------------------------------
// work-item foundation: fail-closed E2E — garbage middle row is DROPPED with a
// named diagnostic; the two flanking valid sessions ingest without throwing.
//
// Locks the wiring of `decodeOrDrop` inside `streamHermes` (work-item boundary):
...
// ---------------------------------------------------------------------------
describe("work-item fail-closed E2E: garbage middle row dropped, flanking sessions ingest", () => {
...
// ---------------------------------------------------------------------------
// work-item regression: event-threading `kind="parent"` must NEVER become a
// parentSessionId.
//
...
// ---------------------------------------------------------------------------
describe("work-item regression: event-level kind=parent edge must not corrupt parentSessionId", () => {
...
// ---------------------------------------------------------------------------
describe("work-item fidelity: message classification (signal kind / drop reason)", () => {
...
describe("work-item fidelity: tool_call entry classification", () => {
...
// ---------------------------------------------------------------------------
// work-item fidelity: malformed-record decode (named diagnostic + drop, no throw)
...
// ---------------------------------------------------------------------------
describe("work-item fidelity: malformed records → named diagnostic + drop (no throw)", () => {
...
// ---------------------------------------------------------------------------
// work-item fidelity E2E: a session_meta marker row in a real state.db is DROPPED
...
// ---------------------------------------------------------------------------
describe("work-item fidelity E2E: session_meta row dropped, conversational turns kept", () => {

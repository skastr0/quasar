...
//
// _indices GC (gcSupersededIndexDirs) runs AFTER optimize(), strictly yield*-sequenced,
// so the new index generation is committed into the manifest before GC reads manifests
// and deletes only provably-unreferenced dirs (never the serving generation).
...
// version regardless of the window [@lancedb/lancedb table.d.ts:88]. deleteUnverified
// stays false so in-flight upsert files are never touched. This reclaims the
// VERSION/data side; gcSupersededIndexDirs handles the _indices generation dirs.
const VERSION_RETENTION_MS = 10 * 60 * 1000;

// Optimize+GC throttle (row-count delta OR wall-clock — grounded values, not invented ratios).
// OPTIMIZE_ROW_DELTA: a 10k-row unindexed tail costs ~17ms at query time (proven:
// 1.1s for full ~660k-row brute-scan → 1.1s × 10000/660000 ≈ 17ms), which is within
// the "instant" bar; so 10k bounds the flat-scanned tail to sub-20ms before the next fold.
  ...
  Effect.gen(function* () {
    ...
    const maintainTable = (
      ...
      Effect.gen(function* () {
        ...
        // manifest AND prunes superseded version manifests/data), THEN gcSupersededIndexDirs

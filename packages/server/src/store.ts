...
const migrate = (db: Database): void => {
  ...
  // corpus cleanup stays in work-item. Safe to re-run on the live data volume.
  ...
  // Idempotent column add for parent-session lineage . Nullable, so
  ...
  // Idempotent column add for search index watermark . NULL means
